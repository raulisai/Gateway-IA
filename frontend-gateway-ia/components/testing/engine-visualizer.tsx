            ...
            <div className="relative z-10 w-full max-w-2xl space-y-6">
                ...
                <motion.div
                    className={cn(
                        ...
                        status === "classifying" ? "bg-blue-500/10 border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.1)]" : "bg-muted/40 border-border hover:border-blue-500/30",
                ...
                >
                    <div className="flex items-center gap-4">
                        <div className={cn("p-2.5 rounded-xl transition-colors", status === "classifying" ? "bg-blue-600 text-white shadow-lg shadow-blue-500/20" : "bg-muted text-muted-foreground")}>
                            <Brain className="w-5 h-5" />
                        </div>
                        <div className="flex-1">
                            <h3 className="font-semibold text-foreground">Input Analysis</h3>
                            <p className="text-xs text-muted-foreground">Scanning prompt complexity & requirements</p>
                        </div>
                                ...
                                <div className="mt-4 pt-4 border-t border-border space-y-3">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-muted/50 p-3 rounded-lg border border-border">
                                            ...
                                            <p className="text-sm font-semibold text-blue-600 dark:text-blue-400 capitalize">{routingInfo?.complexity_level || "Analyzing..."}</p>
                                        </div>
                                        <div className="bg-muted/50 p-3 rounded-lg border border-border">
                                            <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider mb-1">Score</p>
                                            <p className="text-sm font-semibold text-blue-600 dark:text-blue-400">{routingInfo?.complexity_score?.toFixed(2) || "0.00"}</p>
                                        </div>
                                    </div>
                ...
                <div className="relative">
                    ...
                    <motion.div
                        className={cn(
                            "group cursor-pointer flex flex-col p-4 rounded-xl border transition-all duration-300",
                            status === "routing" ? "bg-purple-500/10 border-purple-500/50 shadow-[0_0_15px_rgba(168,85,247,0.1)]" : "bg-muted/40 border-border hover:border-purple-500/30",
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <div className={cn("p-2.5 rounded-xl transition-colors", status === "routing" ? "bg-purple-600 text-white shadow-lg shadow-purple-500/20" : "bg-muted text-muted-foreground")}>
                                <Network className="w-5 h-5" />
                            </div>
                            <div className="flex-1">
                                <h3 className="font-semibold text-foreground">Intelligent Routing</h3>
                                <div className="flex items-center gap-2">
                                    <p className="text-xs text-muted-foreground">Matching best model</p>
                                    {routingInfo && (
                                        <Badge variant="outline" className="text-[10px] py-0 h-4 border-purple-500/30 text-purple-600 dark:text-purple-300">
                            ...
                            {expandedStage === "routing" && (
                                ...
                                >
                                    <div className="mt-4 pt-4 border-t border-border space-y-4">
                                        <div>
                                            <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider mb-2">Scored Candidates</p>
                                            <div className="space-y-2">
                                                {(routingInfo?.proposed_models || [
                                                    ...
                                                    <div key={model.id} className="flex items-center justify-between p-2 rounded-lg bg-muted/30 border border-border">
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-1.5 h-1.5 rounded-full bg-purple-500/50" />
                                                            <div>
                                                                <p className="text-[11px] font-medium text-foreground">{model.name}</p>
                                                                <p className="text-[9px] text-muted-foreground uppercase tracking-tighter">{model.provider}</p>
                                                            </div>
                                                        </div>
                                                        <div className="text-right">
                                                            ...
                                                            <p className="text-[10px] text-purple-600 dark:text-purple-400 font-mono mt-1">{model.score.toFixed(1)}%</p>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                ...
                <div className="relative">
                    ...
                    <motion.div
                        className={cn(
                            "group cursor-pointer flex flex-col p-4 rounded-xl border transition-all duration-300",
                            status === "executing" ? "bg-emerald-500/10 border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.1)]" :
                                status === "complete" ? "bg-emerald-500/5 dark:bg-emerald-500/10 border-emerald-500/20 dark:border-emerald-500/40" : "bg-muted/40 border-border hover:border-emerald-500/30",
                    ...
                    >
                        <div className="flex items-center gap-4">
                            <div className={cn("p-2.5 rounded-xl transition-colors",
                                status === "executing" || status === "complete" ? "bg-emerald-600 dark:bg-emerald-500 text-white shadow-lg shadow-emerald-500/20" : "bg-muted text-muted-foreground")}>
                        ...
                        <AnimatePresence>
                            {expandedStage === "execution" && status !== "idle" && (
                                ...
                                >
                                    <div className="mt-4 pt-4 border-t border-border space-y-3">
                                        <div className="flex items-center justify-between text-xs p-2 bg-muted/20 rounded-lg">
                                            <span className="text-muted-foreground">Active Provider</span>
                                            <span className="font-mono text-emerald-600 dark:text-emerald-400 uppercase">{routingInfo?.provider || "---"}</span>
                                        </div>
                                        <div className="flex items-center justify-between text-xs p-2 bg-muted/20 rounded-lg">
                                            <span className="text-muted-foreground">Inference Mode</span>
                                            <span className="font-mono text-emerald-600 dark:text-emerald-400">Optimized Stream</span>
                                        </div>
                                    </div>
                                </motion.div>
                            )}
